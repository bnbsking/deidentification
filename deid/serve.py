...
from deid.llm_pydantic import schema_to_model


cfg = yaml.safe_load(open(os.getenv("API_KEYS_PATH"), "r"))
llm = AzureOpenAIChatAPI(
...
@app.post("/cloud_api")
def cloud_api(r: CloudAPIRequest):
    response_format = schema_to_model("custom", r.response_format_dict) \
        if r.response_format_dict else None
    return llm.run(prompt=r.prompt, response_format=response_format)


@app.post("/async_cloud_api")
def async_cloud_api(r: List[CloudAPIRequest]):
